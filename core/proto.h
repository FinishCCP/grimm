...
#define MWGNodeMsg_NewTip(macro) \
    macro(Block::SystemState::Full, Description)

#define MWGNodeMsg_GetHdr(macro) \
    macro(Block::SystemState::ID, ID)

#define MWGNodeMsg_Hdr(macro) \
    macro(Block::SystemState::Full, Description)

#define MWGNodeMsg_GetHdrPack(macro) \
    macro(Block::SystemState::ID, Top) \
    macro(uint32_t, Count)

#define MWGNodeMsg_HdrPack(macro) \
    macro(Block::SystemState::Sequence::Prefix, Prefix) \
    macro(std::vector<Block::SystemState::Sequence::Element>, vElements)

#define MWGNodeMsg_DataMissing(macro)

#define MWGNodeMsg_Status(macro) \
    macro(uint8_t, Value)

#define MWGNodeMsg_GetBody(macro) \
    macro(Block::SystemState::ID, ID)

#define MWGNodeMsg_GetBodyPack(macro) \
    macro(Block::SystemState::ID, Top) \
    macro(uint8_t, FlagP) \
    macro(uint8_t, FlagE) \
...
#define MWGNodeMsg_Body(macro) \
    macro(BodyBuffers, Body)

#define MWGNodeMsg_BodyPack(macro) \
    macro(std::vector<BodyBuffers>, Bodies)

#define MWGNodeMsg_GetProofState(macro) \
    macro(Height, Height)

#define MWGNodeMsg_GetCommonState(macro) \
    macro(std::vector<Block::SystemState::ID>, IDs)

#define MWGNodeMsg_GetProofKernel(macro) \
    macro(Merkle::Hash, ID)

#define MWGNodeMsg_GetProofKernel2(macro) \
    macro(Merkle::Hash, ID) \
    macro(bool, Fetch)

#define MWGNodeMsg_GetProofUtxo(macro) \
...
#define MWGNodeMsg_GetProofChainWork(macro) \
    macro(Difficulty::Raw, LowerBound)

#define MWGNodeMsg_ProofKernel(macro) \
    macro(TxKernel::LongProof, Proof)

#define MWGNodeMsg_ProofKernel2(macro) \
    macro(Merkle::Proof, Proof) \
    macro(Height, Height) \
    macro(TxKernel::Ptr, Kernel)

#define MWGNodeMsg_ProofUtxo(macro) \
    macro(std::vector<Input::Proof>, Proofs)

#define MWGNodeMsg_ProofState(macro) \
    macro(Merkle::HardProof, Proof)

#define MWGNodeMsg_ProofCommonState(macro) \
    macro(Block::SystemState::ID, ID) \
    macro(Merkle::HardProof, Proof)

#define MWGNodeMsg_ProofChainWork(macro) \
    macro(Block::ChainWorkProof, Proof)

#define MWGNodeMsg_Login0(macro) \
    macro(ECC::Hash::Value, CfgChecksum) \
    macro(uint8_t, Flags)

#define MWGNodeMsg_Login(macro) \
    macro(std::vector<ECC::Hash::Value>, Cfgs) \
    macro(uint32_t, Flags)

#define MWGNodeMsg_Ping(macro)
#define MWGNodeMsg_Pong(macro)

#define MWGNodeMsg_NewTransaction(macro) \
    macro(Transaction::Ptr, Transaction) \
    macro(bool, Fluff)

#define MWGNodeMsg_HaveTransaction(macro) \
    macro(Transaction::KeyType, ID)

#define MWGNodeMsg_GetTransaction(macro) \
    macro(Transaction::KeyType, ID)

#define MWGNodeMsg_Bye(macro) \
    macro(uint8_t, Reason)

#define MWGNodeMsg_PeerInfoSelf(macro) \
    macro(uint16_t, Port)

#define MWGNodeMsg_PeerInfo(macro) \
    macro(PeerID, ID) \
    macro(io::Address, LastAddr)

#define MWGNodeMsg_GetTime(macro)

#define MWGNodeMsg_Time(macro) \
    macro(Timestamp, Value)

#define MWGNodeMsg_GetExternalAddr(macro)

#define MWGNodeMsg_ExternalAddr(macro) \
    macro(uint32_t, Value)

#define MWGNodeMsg_BbsMsgV0(macro) \
    macro(BbsChannel, Channel) \
    macro(Timestamp, TimePosted) \
    macro(ByteBuffer, Message)

#define MWGNodeMsg_BbsMsg(macro) \
    macro(BbsChannel, Channel) \
    macro(Timestamp, TimePosted) \
    macro(ByteBuffer, Message) \
    macro(Bbs::NonceType, Nonce)

#define MWGNodeMsg_BbsHaveMsg(macro) \
    macro(BbsMsgID, Key)

#define MWGNodeMsg_BbsGetMsg(macro) \
    macro(BbsMsgID, Key)

#define MWGNodeMsg_BbsSubscribe(macro) \
    macro(BbsChannel, Channel) \
    macro(Timestamp, TimeFrom) \
    macro(bool, On)

#define MWGNodeMsg_BbsResetSync(macro) \
    macro(Timestamp, TimeFrom)

#define MWGNodeMsg_BbsPickChannelV0(macro)

#define MWGNodeMsg_BbsPickChannelResV0(macro) \
    macro(BbsChannel, Channel)

#define MWGNodeMsg_SChannelInitiate(macro) \
    macro(ECC::uintBig, NoncePub)

#define MWGNodeMsg_SChannelReady(macro)

#define MWGNodeMsg_Authentication(macro) \
    macro(PeerID, ID) \
    macro(uint8_t, IDType) \
    macro(ECC::Signature, Sig)

#define MWGNodeMsg_MacroblockGet(macro) \
    macro(Block::SystemState::ID, ID) \
    macro(uint8_t, Data) \
    macro(uint64_t, Offset)

#define MWGNodeMsg_Macroblock(macro) \
    macro(Block::SystemState::ID, ID) \
    macro(ByteBuffer, Portion) \
    macro(uint64_t, SizeTotal)

#define MWGNodeMsg_GetUtxoEvents(macro) \
    macro(Height, HeightMin)

#define MWGNodeMsg_UtxoEvents(macro) \
    macro(std::vector<UtxoEvent>, Events)

#define MWGNodeMsg_GetBlockFinalization(macro) \
    macro(Height, Height) \
    macro(Amount, Fees)

#define MWGNodeMsg_BlockFinalization(macro) \
    macro(Transaction::Ptr, Value)

#define MWGNodeMsgsAll(macro) \
    /* general msgs */ \
    macro(0x00, Login0) \
    macro(0x01, Bye) \
    macro(0x02, Ping) \
...
#define THE_MACRO1(code, msg) \
    ...
    { \
        ...
        MWGNodeMsg_##msg(THE_MACRO2) \
        template <typename Archive> void serialize(Archive& ar) { ar MWGNodeMsg_##msg(THE_MACRO3); } \
        msg(Zero_ = Zero) { MWGNodeMsg_##msg(THE_MACRO4) } /* default c'tor, zero-init everything */ \
        msg(Uninitialized_) { } /* don't init members */ \
        msg(MWGNodeMsg_##msg(THE_MACRO5) Unused_ = Unused) { MWGNodeMsg_##msg(THE_MACRO6) } /* explicit init */ \
    }; \
    struct msg##_NoInit :public msg { \
        msg##_NoInit() :msg(Uninitialized) {} \
    }; \

    MWGNodeMsgsAll(THE_MACRO1)
#undef THE_MACRO1
#undef THE_MACRO2
#undef THE_MACRO3
#undef THE_MACRO4
